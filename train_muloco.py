...
# MuLoCo K=1 arguments
parser.add_argument("--outer-lr", type=float, default=0.7, help="Outer Nesterov SGD learning rate")
parser.add_argument("--outer-momentum", type=float, default=0.6, help="Outer Nesterov SGD momentum")
parser.add_argument("--sync-interval", type=int, default=20, help="Inner steps between outer updates (H)")
...
class GPT(nn.Module):
    ...
    def setup_optimizer(self):
        ...
        inner_optimizer = Factory(param_groups)
        # Wrap with MuLoCo K=1
        optimizer = MuLoCoWrapper(
            inner_optimizer,
...
class MuLoCoWrapper:
    """
    MuLoCo K=1: wraps any optimizer with outer Nesterov SGD.

    Every `sync_interval` inner optimizer steps, computes a pseudogradient
    (snapshot - current params) and applies an outer Nesterov SGD update.

    This implements Algorithm 1 from the MuLoCo paper with K=1 workers.
    """

    def __init__(self, inner_optimizer, outer_lr=0.7, outer_momentum=0.6, sync_interval=20):
...
# wandb
run_name = args.run_name if args.run_name else time.strftime("%Y%m%d_%H%M%S")
_wandb_kwargs = {"project": "slow-run-muloco", "name": run_name}
if args.wandb_group:
...
print0(f"  [MuLoCo K=1] outer_lr={args.outer_lr}, outer_momentum={args.outer_momentum}, sync_interval={args.sync_interval}")
print0(f"-----------------------")
